...
class CheckerEnvForMCTS:
    ...
    def step(self, move):
        ...
        terminated, winning_player = self.check_termination()
    ...
    def check_termination(self):
        ...
        if self.active == WHITE and terminated:
            winning_player = "Black"
        elif self.active == BLACK and terminated:
            winning_player = "White"

        return end, winning_player
...
def copyEnvMCtoMCTS(instanceCheckerEnvMC):
    ...
    output.backward = instanceCheckerEnvMC.backward

    output.pieces = instanceCheckerEnvMC.pieces

    output.max_steps = instanceCheckerEnvMC.max_steps

    output.active = instanceCheckerEnvMC.active
    output.passive = instanceCheckerEnvMC.passive

    output.num_steps = instanceCheckerEnvMC.num_steps

    output.empty = instanceCheckerEnvMC.empty

    output.jump = instanceCheckerEnvMC.jump
    output.mandatory_jumps = instanceCheckerEnvMC.mandatory_jumps
