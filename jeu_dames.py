...
class Damier(tk.Tk):
    ...
    def creer_damier(self):
        for i in range(self.nb_cases):
            for j in range(self.nb_cases):
                ...
                canvas = tk.Canvas(self, width=self.taille_case, height=self.taille_case, bd=0, highlightthickness=0,
                                   bg=couleur)
    ...
    def placer_pion_for_test(self):
        ...
        image_path = r'.\media\picture\pion_noir.png'
        ...
        image_path = r'.\media\picture\pion_blanc.png'
    ...
    def pion_peut_manger(self, moi, lui, x1,
                         y1):  # check toutes les possibilités du tableau, choper le max random parmi les meilleurs
        ...
        for l in self.liste:
            ...
            # print((x1+l[0])
            # print((y1+l[1])
            if (x1 + l[0] >= 1 and x1 + l[0] <= 6 and y1 + l[1] >= 1 and y1 + l[1] <= 6 and self.plateau[x1 + l[0] * 2][
                y1 + l[1] * 2].type == "empty" and self.plateau[x1 + l[0]][
                y1 + l[1]].type == lui):  # vérif qu'on est pas au bord : carré interne de 6*6
                # print(("x1_int:"+ str(x1))
                # print(("y1_int:"+ str(y1))
                # print((self.length)
    ...
    def clic_relache(self, event):
        ...
        if ((ligne != self.coord_appui[0] or self.coord_appui[
            1] != colonne) and ligne <= 7 and colonne <= 7 and ligne >= 0 and colonne >= 0 and self.checker_pion(
                self.coord_appui[0], self.coord_appui[1], ligne, colonne)):
                ...
                for i in range(0, len(self.listemax)):
                    ...
                    if (np.all(coord_predites == [x2, y2])):
                        ...
                        if (unefois):
                            ...
                            unefois = False
                        copie_liste.append(
                            [self.listemax[i][0][1:], self.listemax[i][1]])  # remove toujours la premiere occurence
